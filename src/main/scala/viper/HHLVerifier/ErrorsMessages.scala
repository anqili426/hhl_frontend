...
case class LoopRuleExistsInvErr(expr: Expr) extends ErrorMsg(expr) {
  override def text: String = "The following invariant used in a exist rule might not hold:"
}

case class LoopModularErr(expr: Expr) extends ErrorMsg(expr) {
  override def text: String = "[EXISTS RULE] The following modular invariant might not hold:"
}