...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val programAbsPath = "/Users/anqili/Desktop/thesis/frontend-demo/src/main/scala/viper/HHLVerifier/program.txt"
    val program = scala.io.Source.fromFile(programAbsPath).mkString
    try {
      val res = fastparse.parse(program, Parser.program(_))
      if (res.isSuccess) {
        // Generate code if parsed.success
        // Get the parse tree
        val parsedProgram = res.get.value.content
