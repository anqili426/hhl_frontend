...
object ViperRunner {
  ...
  def runSiliconAndCarbon(program: Program) = {
    val consistencyErrors = program.checkTransitively
    if (consistencyErrors.nonEmpty) {
      consistencyErrors.foreach(err => printMsg(err.readableMessage))
      sys.exit(1)
      ...
      val carbonRes = Future[VerificationResult] {
        printMsg("Carbon has been started. ")
        carbon.verify(program)
      }

      val siliconRes = Future[VerificationResult] {
        printMsg("Silicon has been started. ")
          ...
          result match {
            ...
            case ResFailure(err) =>
              ...
              if (!resPromise.isCompleted) {
                ...
                if (!siliconRes.isCompleted) Await.result(siliconRes, 180.seconds) // Wait for silicon to terminate
          ...
          result match {
            ...
            case ResFailure(err) =>
              if (!resPromise.isCompleted) {
                ...
                printMsg("Silicon: ")
                err.foreach(e => printMsg(e.readableMessage))
                if (!carbonRes.isCompleted) Await.result(carbonRes, 180.seconds) // Wait for carbon to terminate
