        ...
        case loop@WhileLoopStmt(cond, body, invWithHints, decr, rule) =>
            ...
            if (autoSelectRules && rule == "unspecified") {
              ...
              if (!isAutoSelected && rule == "existsRule") {
                val invHasTopExists = normalizedInv.exists(i => checkHasTopExists(i))
              ...
              // Assert I(0)
              if (normalizedInv.nonEmpty) {
                for ((normalizedInv, i) <- normalizedInv.zipWithIndex) {
                  newStmts = newStmts :+ vpr.Assert(translateExp(normalizedInv, null, currStates, loopFailureStates))(info = LoopEntryPointErr(inv(i)).getMsg)
                }
              }
