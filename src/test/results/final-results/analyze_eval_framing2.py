#!/usr/bin/env python3
"""
Compare syntactic-both runtimes with framing ENABLED vs DISABLED, where BOTH inputs
are in the reduced CSV format, e.g.:

"Test case name","Number of triples","SMT mode","Runtime total (s)",...,"Test result",...

Usage:
  python3 analyze_eval_framing2.py \
    --with "src/test/evaluation/output_frame*.csv" \
    --without "src/test/evaluation/output*.csv" \
    --outdir "src/test/evaluation/plots_framing_cmp"

Notes:
- Compares ONLY SMT mode == "both"
- Averages over repetitions per test for both datasets
...
# ----------------------------
# Reading reduced-format CSVs (both configs)
# ----------------------------
def read_reduced(patterns: List[str], config_label: str) -> pd.DataFrame:
...
def print_overview_tables(agg_with: pd.DataFrame, agg_wo: pd.DataFrame, merged: pd.DataFrame, eps: float) -> None:
    ...
    tab = pd.DataFrame([
        summarize(agg_with, "with_framing"),
        summarize(agg_wo,   "without_framing"),
    ])
    ...
    helps = int(np.sum(su > 1.0 + eps))
    hurts = int(np.sum(su < 1.0 - eps))
...
# Plots
# ----------------------------
def plot_scatter(merged: pd.DataFrame, outdir: Path) -> None:
    x = merged["rt_with"].to_numpy(dtype=float)
    y = merged["rt_without"].to_numpy(dtype=float)
...
# ----------------------------
def main() -> None:
    ap = argparse.ArgumentParser(description="Compare syntactic-both with vs without framing (reduced CSV format).")
    ap.add_argument(
        "--with",
        dest="with_globs",
        nargs="+",
        default=["src/test/evaluation/output_frame*.csv"],
        help="Glob(s) for 'with framing' reduced CSVs (default: src/test/evaluation/output_frame*.csv).",
    )
    ap.add_argument(
        "--without",
        dest="without_globs",
        nargs="+",
        required=True,
        help="Glob(s) for 'without framing' reduced CSVs (e.g., output*.csv).",
    )
    ...
    df_with = read_reduced(args.with_globs, config_label="with_framing")
    df_wo   = read_reduced(args.without_globs, config_label="without_framing")
